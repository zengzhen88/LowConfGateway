...
void *_av_md5_alloc(void)
{
    return (void *)av_mallocz(sizeof(struct AVMD5));
}

void _av_md5_free(void *oObj) {
    free(oObj);
}
...
void _av_md5_init(void *oObj)
{
    AVMD5 *ctx  = (AVMD5 *)oObj;
    ctx->len     = 0;
...
void _av_md5_update(void *oObj, const uint8_t *src, size_t len)
{
    AVMD5 *ctx = (AVMD5 *)oObj;
    const uint8_t *end;
    int j;
    ...
    if (j) {
        int cnt = FFMIN((int32_t)len, 64 - j);
...
void _av_md5_final(void *oObj, uint8_t *dst)
{
    AVMD5 *ctx = (AVMD5 *)oObj;
    int i;
    uint64_t finalcount = av_le2ne64(ctx->len << 3);

    _av_md5_update(ctx, (const uint8_t *)"\200", 1);
    while ((ctx->len & 63) != 56)
        _av_md5_update(ctx, (const uint8_t *)"", 1);

    _av_md5_update(ctx, (const uint8_t *) &finalcount, 8);

    for (i = 0; i < 4; i++)
        AV_WL32(dst + 4 * i, ctx->ABCD[3 - i]);
}

void _av_md5_sum(uint8_t *dst, const uint8_t *src, size_t len)
{
    AVMD5 ctx;

    _av_md5_init(&ctx);
    _av_md5_update(&ctx, src, len);
    _av_md5_final(&ctx, dst);
}
#endif
