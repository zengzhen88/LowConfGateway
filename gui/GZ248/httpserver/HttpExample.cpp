...
void HttpExample::start(string download, int32_t port)
{
    printf ("dlowkjasdf:%s\n", download.c_str());
    ...
    _httpServer->addStaticWebDir(_download, sstream.str());
    _httpServer->startHttp(port);//8080);
...
void HttpExample::handleHttpBigFileUpload(shared_ptr<HttpReqMsg> &httpMsg)
{
    ...
    string filePathPrefix = _download;//"/data/";
...
void HttpExample::handleHttpDownloadFile(shared_ptr<HttpReqMsg> &httpMsg)
{
    ...
    string filePath = _download + fileName;
    // printf ("filePath:%s\n", filePath.c_str());
