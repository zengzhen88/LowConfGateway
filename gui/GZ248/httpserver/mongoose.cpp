...
/*
    ...
    返回值：
    1、返回值为0时，表示当前传入的分析数据连head头都不全，没有分析价值，这部分数据应该原封不动的从下一轮数据再传进来
                                  ...
                                  struct mg_str *head, struct mg_str *extraData, bool *partCompleted)
{
  *partCompleted = false;
  ...
  if (!(*partCompleted) && extraData)
  {
      extraData->ptr = &s[b2];
      extraData->len = max - b2;
  }
  // return the end of body
  return (*partCompleted) ? b2 + 2 : b2;
}
...
int mg_http_parse(const char *s, size_t len, struct mg_http_message *hm) {
  ...
  */
  ...
  // my added code, if is not chunk and have no "Content-Length" in head
  if (hm->body.len == (size_t) ~0 && !is_response && !mg_is_chunked(hm))
  {
    hm->body.len = 0;
    hm->message.len = (size_t) req_len;
  }
...
void w_mg_http_reply(struct mg_connection *c, int code, const char *headers,
    ...
    mg_send(c, body, bodyLen > 0 ? (size_t) bodyLen : 0);
}
...
static void http_cb(struct mg_connection *c, int ev, void *evd, void *fnd) {
  if (ev == MG_EV_READ || ev == MG_EV_CLOSE) {
    ...
    for (;;) {
      // the returned n is http head len, if head is not received finished, return 0
      ...
      if (n < 0 && ev == MG_EV_READ) {
        ...
        if (!is_chunked)
        {
            hm.chunk = mg_str_n((char *) &c->recv.buf[n], c->recv.len - (size_t) n);
            mg_call(c, MG_EV_HTTP_CHUNK, &hm);
        }
        ...
        if (n > 0 && !is_chunked) {
          // if you wirte "c->recv -= hm.chunk.len" in external code, you can receive unlimited data size,
          // and MG_EV_HTTP_MSG callback will only be triggered when fd is closed
          hm.chunk = mg_str_n((char *) &c->recv.buf[n], c->recv.len - (size_t) n);
          mg_call(c, MG_EV_HTTP_CHUNK, &hm);
        }
        break;
...
void w_sync_mg_send(struct mg_connection *c, const char* data, int len)
{
  ...
  long n = c->is_tls ? mg_tls_send(c, data, len) : mg_sock_send(c, data, len);
  if (n < 0) {
    c->is_closing = 1;  // Error, or normal termination
  }
}
...
static void accept_conn(struct mg_mgr *mgr, struct mg_connection *lsn) {
  ...
  } else if ((long) fd >= EPOLL_MAX_FD_NUM) {
...
void mg_mgr_poll(struct mg_mgr *mgr, int ms) {
  ...
  for (c = mgr->conns; c != NULL; c = tmp) {
    ...
    /*
      If the socket is associated with ssl, then is_readable is always true, even if there is no data 
      actually coming, which will cause blocking at SSL_read for a long time， add "c->is_draining && c->is_tls"
      can make code do not run "read_conn" when the socket will be close
      you can also call "setsockopt(c->fd, SOL_SOCKET, SO_RCVTIMEO, (char *)&timeout, sizeof(struct timeval))" to
      set blocking timeout of socket
    */
    if (c->is_resolving || c->is_closing) {
      // Do nothing
    } else if (c->is_draining && c->is_tls) {
